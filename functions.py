...
def return_keyword(review_data):
    ...
    # JKS, JX_중요조사들
    josa = set(['JKS', 'JX'])  # 품사 중 조사에 대한 표현 저장 #+JKO, JKB, JKG, JKV, JKC, JC
...
def review_summarization(data):
    ...
    for word in keyword:
        review_data_list = []
        for i in range(len(review_data)):
            if word in review_data.loc[i,'review']:
                ws, value = DNN_func(review_data.loc[i,'review'])
                if 2 < len(review_data.loc[i,'review']) < 30:
                    ...
                    if temp_sent:
                        ...
                        if doc2vec is not None:
                            review_data_list.append([i, review_data.loc[i,'review'], doc2vec,
...
def similarity_and_major_similar_sentence(review_data, vocab_sorted, selected_review,
    ...
    for i in range(len(review_data)):
        for word in all_line_of_review:
            if 2 < len(review_data.loc[i,'review']) < 40 and word in review_data.loc[i,'review'] and review_data.loc[i,'review'] != selected_review:
